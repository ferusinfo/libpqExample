...
/* Use it to execute a SQL statement*/
...
printf("\n---------------------------------\n");
 ...
 printf("Zakonczenie pracy = 0\nPolacz sie z baza = 1\nStworz tabele = 2\nUsun tabele = 3\nStworz wyzwalacz = 4\nUsun wyzwalacz = 5\nDodaj rekord do tabeli = 6\nModyfikuj rekord = 7\nUsun rekord = 8\nWyswietl zawartosc tabeli = 9\nWyszukiwanie w tabeli = 10\n");
       ...
       printf("Twoj wybor: ");
        ...
        case 1:
  conn = PQconnectdb("host=changeme port=5432 dbname=changeme user=changeme password=changeme");
  ...
  printf("---------------------------------\n");
	...
	printf("\n");
		...
		printf("\n---------------------------------\n");
		executeSQL(conn, "DROP TABLE osoba");
		 executeSQL(conn, "CREATE TABLE osoba(id_osoba INTEGER PRIMARY KEY, imie VARCHAR, nazwisko VARCHAR, wiek INT, data_ur DATE, pensja MONEY)");
		...
		printf("\n---------------------------------\n");
	executeSQL(conn, "CREATE OR REPLACE FUNCTION wiek() RETURNS TRIGGER AS $$ BEGIN IF NEW.wiek < 16 THEN RAISE EXCEPTION 'OSOBA JEST ZA MLODA'; END IF; IF NEW.pensja::NUMERIC < 0 THEN RAISE EXCEPTION 'Pensja nie moze byc mniejsza niz 0!'; END IF; RETURN NEW; END; $$ LANGUAGE 'plpgsql';");
executeSQL(conn, "CREATE TRIGGER wyzwalacz BEFORE INSERT OR UPDATE ON osoba FOR EACH ROW EXECUTE PROCEDURE wiek();");
		printf("\n");
		printf("\n");
		printf("\n");printf("\n");
        break;
		...
		printf("\n---------------------------------\n");
						executeSQL(conn, "DROP TRIGGER wyzwalacz ON osoba");
		executeSQL(conn, "DROP FUNCTION wiek()");
		printf("\n");
		printf("\n");
		printf("\n");printf("\n");
        break;
		...
		executeSQL(conn, "SELECT * FROM osoba");
		...
		printf("\n---------------------------------\n");
			...
			switch(z){
				...
				executeSQL(conn, zapytanie);printf("\n");printf("\n");
				break;
